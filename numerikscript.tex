...
Vo­r­aus­set­zungen für diesen Abschnitt: \\
$a = x_0 < x_1 < ... < x_n = b$, \\
$h_i= x_i-x_{i-1}$, \\
...
\begin{lemma}
Unter den Vo­r­aus­set­zungen von (13.1) gilt für  $s'(x_i) = \tau_i$:
...
Falls $m \gg n \medspace \rightarrow \medspace 2m ( n + (n-1) + \cdots + 1 ) \approx mn^2)$
\end{nothing}
...
Hierbei sind die $\varphi_1, ..., \varphi_n$ gegebene Funktionen und $x_1, ..., x_n$ unbekannte Parameter, wobei $m \gg n$.\\
...
$$f(t) = x_2t + x_1.$$
Bezeichne nun $x_2$ mit $m$ und $x_1$ mit $c$, um die übliche Geradendarstellung zu erhalten:
$$f(t) = mt + c,$$
...
\begin{comment}
Jede Differentialgleichung $k$-ter Ordnung
$$y^{(k)} = f(t, y, y',..., y^{(k-1)})$$
...
\begin{comment*}[Allgemeine Vo­r­aus­set­zungen]
...
Unter obigen Vo­r­aus­set­zungen gilt: Es gibt ein offenes Intervall $I$ mit $t_0 \in I$, sodass genau eine Lösung $y: I \rightarrow \mathbb{R}^d$ existiert, mit
...
ist lokal Lipschitz-stetig, erfüllt also die Vo­r­aus­set­zungen von Picard-Lindelöf. Die eindeutige Lösung $y(t) = (1-t)^{-1}$ existiert auf dem offenen Intervall $I = (-\infty, 1)$ und $t_0 = 0 \in I$. Es ist jedoch $\lim_{t \nearrow 1} y(t) = + \infty$. 
\end{example}

Für numerische Verfahren ist es wichtig wie sich Störungen der Anfangswerte auf die Lösung auswirken.

\begin{theorem}
Zusätzlich zu den allgemeinen Vo­r­aus­set­zungen erfülle für $\langle \cdot, \cdot \rangle$ ein Skalarprodukt auf $\mathbb{C}^d$ und $\Vert \cdot \Vert$ die induzierte Norm die rechte Seite $f: [t_0, T] \rightarrow \mathbb{C}^d$ $f$ für ein $l$ folgende einseitige Lipschitz-Bedingung
...
Allgemeine Iterationsvorschrift (explizites Eulerverfahren):\\
$t_n = t_0 + nh$ \\
...
Mit den eben gemachten Vo­r­aus­set­zungen gilt für den Fehler des expliziten Euler-Verfahrens
...
Sei $s=1$, $b_1 = 1$, $c_1 = \nicefrac{1}{2}$
$$ y(t_1) \approx y(t_0) + hf\left(t_0 + \frac{1}{2}h, y\left(t_0 + \frac{1}{2}h\right)\right)$$
Wie berechnet man $y(t_0 + \nicefrac{1}{2}h)$?
Zum Beispiel mit einem Schritt des expliziten Eulerverfahrens
$$y\left(t_0 + \frac{1}{2}h\right) \approx y(t_0) + \frac{1}{2}hf(t_0, y(t_0))$$
\end{example}
...
&Y_1' = f(t_0, y_0), \quad Y_2' = f\left(t_0 + \frac{h}{2}, Y_2\right) \\
&y_1 = y_0 + hY_2'
\end{align*}
Butcher Tableau

...
&= \frac{1}{2} \text{Re}\left( \sum_{l=0}^n e^{i(k+j)lh}e^{i(k+j)\frac{h}{2}} + e^{i(k-j)lh}e^{i(k-j)\frac{h}{2}}\right) &\\
...
\intertext{Es gilt $k(n+1) = \pi$. Behauptung:}
&= \begin{cases}
0 & k \neq j \\
\frac{1}{2} (n+1) & k=j\neq 0 \\
(n+1) & k = j = 0
...
\end{align*}
\begin{description}
  \item[Fall 1:] $k=j=0 \Rightarrow \frac{1}{2} \sum_{l=0}^n(1+1)$
  \item[Fall 2:] $k=j\neq0 \Rightarrow \frac{1}{2} \sum_{l=0}^n\left( e^{i2k(l+\frac{1}{2})h} \right) = \dots$ 
  \item[Fall 3:] $k \neq j$:
    \begin{description}
      \item[Fall 1:] $k+j$ ist gerade und $k-j$ ist gerade 
      \item[Fall 2:] $k+j$ ist ungerade und $k-j$ ist ungerade
    \end{description}
\end{description}
\end{proof}
\end{lemma}
...
\begin{comment*}
$(\thinspace \dot \thinspace, \thinspace \dot \thinspace )$ ist ein Skalarprodukt auf $\mathcal{P}_n$, da 
\begin{enumerate}
  \item[i)] bilinear
  \item[ii)] symmetrisch

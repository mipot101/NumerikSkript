...
\begin{description}
  \item[Beispiel:]
    ...
    Was passiert bei fehlerhafter Startwerten $\tilde{x}_0 = 1+ \epsilon$? \\
    $ \tilde{x}_{n+1} = 10 \tilde{x}_n - 9, \quad \tilde{x}_n = 1+ 10^n \epsilon$
\end{description}
Der Clenshaw-Algorithmus ist stabil, wie im Folgenden gezeigt wird:
\end{comment*}

\begin{theorem}
Für den Clanshaw-Algorithmus mit Fehlern $\epsilon_k$ in der Rekursion, d.h. für 
\begin{align*}
&\tilde{d}_{n+1} = \tilde{d}_{n+2} = 0 &\\
&\tilde{d}_k = c_k + 2x \tilde{d}_{k+1} - \tilde{d}_{k+2} + \epsilon_k, \quad k=n, n-1, ..., 0 &\\
\intertext{Dabei ist $\epsilon_k$ der Rundungsfehler in der k-ten Iteration. Für
$\tilde{p}(x) = \frac{1}{2} (\tilde{d}_0 - \tilde{d}_2)$ gilt:}
& \vert \tilde{p}(x) - p(x) \vert \leq \sum_{j=0}^n \vert \epsilon_j \vert, \quad \text{für } \vert x \vert < 1,
\intertext{wobei $p(x)$ mit (10.10) berechnet wird.}
\end{align*}

\begin{proof}[Beweis]
Setze $\epsilon_k := \tilde{d}_k - d_k$ (für $d_k$ aus (10.10)). Dann gilt:
\begin{align*}
& \epsilon_k = \epsilon_k + 2x \epsilon_{k+1} - \epsilon_{k+2}, \quad \text{für } k=n,...,0 &\\
&\epsilon_{n+1} = 0 \quad \text{und} \quad \epsilon_{n+2} = 0
\end{align*}
Mit Satz (10.10) gilt für $c_k = \epsilon_k$ und $d_k = \epsilon_k$:
\begin{align*}
& \frac{1}{2} (\epsilon_0 -\epsilon_2) = \frac{1}{2} \epsilon_0 + \sum_{j=1}^n \epsilon_j T_j(x) 
\intertext{Da $\vert T_j(x) \vert \leq 1$ für $x \in [1, 1]$ gilt:}
& \vert \tilde{p}(x) - p(x) \vert \overset{\Delta-UGL}{\leq} \frac{1}{2} \vert \epsilon_0 \vert + \sum_{j=1}^n \vert \epsilon_j \vert  
\end{align*}
\end{proof}
\end{theorem}
